...
class SecondaryProduct {
  ...
  /**
   * Delete a secondary product and restore all primary product inventory
   * that was consumed when this product was produced.
   *
   * How restoration works:
   * - Primary products have no per-batch stock tracking — they hold a single
   *   scalar `quantity`. Restoring inventory therefore means incrementing the
   *   primary's quantity by (recipeQty × currentSecondaryStock) for each
   *   component in the recipe.
   * - `currentSecondaryStock` is the quantity field on this document at the
   *   time of deletion — i.e. the units that are still in stock and whose
   *   raw-material cost has not yet been offset by tertiary production.
   * - All reads happen before all writes inside a single Firestore transaction
   *   so the restore+delete is atomic and idempotent: retrying after a
   *   successful delete finds the document gone and aborts without
   *   double-restoring anything.
   *
   * Guard: the method still refuses deletion if this product is referenced by
   * any tertiary product (existing behavior preserved).
   */
  static async delete(id) {
    try {
      // ── Pre-flight: refuse if any tertiary product uses this secondary ──
      // This check runs outside the transaction because it requires a
      // collection-wide scan that cannot be expressed as a transactional read.
      // A tiny race window exists (a tertiary could be created between this
      // check and the transaction), but that is the same exposure as the
      // original code and is acceptable given typical usage patterns.
      const tertiarySnapshot = await db.collection("tertiaryProducts").get();
      for (const doc of tertiarySnapshot.docs) {
        const data = doc.data();
        if (
          data.components &&
          data.components.some((comp) => comp.productId === id)
        ) {
          throw new Error(
            "Cannot delete secondary product that is used in tertiary products",
          );
      ...
      await db.runTransaction(async (t) => {
        ...
        if (!doc.exists) {
          // Already deleted — idempotent no-op; aborts without any writes.
          throw new Error("Secondary product not found or already deleted");
        }
        ...
        // Build the list of primary products to restore and the amounts.
        // Only restore for the stock that is still in hand (currentQty).
        // Damaged units have already been accounted for in damagedQuantity
        // and are NOT part of currentQty, so they are correctly excluded.
        const primaryRefs = [];
        const restoreAmounts = [];

        if (currentQty > EPSILON && components.length > 0) {
          for (const comp of components) {
            if (!comp.productId) continue;
            const restoreQty = roundQty(parseFloat(comp.quantity) * currentQty);
            if (restoreQty <= EPSILON) continue;
            primaryRefs.push(
              db.collection("primaryProducts").doc(comp.productId),
            );
            restoreAmounts.push(restoreQty);
          }
        }

        // Read all primary product documents that need updating.
        const primaryDocs =
          primaryRefs.length > 0
            ? await Promise.all(primaryRefs.map((ref) => t.get(ref)))
            : [];

        // ── PHASE 2: writes ───────────────────────────────────────────────
        primaryDocs.forEach((primaryDoc, i) => {
          if (!primaryDoc.exists) return; // primary deleted — skip
          const primaryData = primaryDoc.data();
          const newQty = roundQty(
            toNumber(primaryData.quantity || 0) + restoreAmounts[i],
          );
          t.update(primaryRefs[i], {
            quantity: newQty,
            updatedAt: new Date(),
          });
        });

        // Delete the secondary product itself.
        t.delete(docRef);
      });
